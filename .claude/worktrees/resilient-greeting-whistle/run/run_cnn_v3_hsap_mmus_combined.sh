...
# Build combined Hsap+Mmus training files (cached under data/combined_Hsap_Mmus/).
if (( DONOR_LEN >= ACCEPTOR_LEN )); then _combined_len=${DONOR_LEN}; else _combined_len=${ACCEPTOR_LEN}; fi
_combined_dir="${DATA_ROOT}/combined_Hsap_Mmus/raw"
_combined_pos="${_combined_dir}/${_combined_len}bp.err"
_combined_neg="${_combined_dir}/${_combined_len}bp.neg.err"
if [[ ! -f "${_combined_pos}" || ! -f "${_combined_neg}" ]]; then
	echo "[cnn_v3_hsap_mmus_combined.sh] Building combined training data (Hsap+Mmus, ${_combined_len}bp)..."
	mkdir -p "${_combined_dir}"
	cat "${DATA_ROOT}/Hsap/raw/${_combined_len}bp.err" \
	    "${DATA_ROOT}/Mmus/raw/${_combined_len}bp.err" > "${_combined_pos}"
	cat "${DATA_ROOT}/Hsap/raw/${_combined_len}bp.neg.err" \
	    "${DATA_ROOT}/Mmus/raw/${_combined_len}bp.neg.err" > "${_combined_neg}"
	echo "[cnn_v3_hsap_mmus_combined.sh] Combined files created."
fi
TRAIN_POS_PATH="${_combined_pos}"
TRAIN_NEG_PATH="${_combined_neg}"
# Use Hsap best tuned config for hyperparameters.
DONOR_TUNED_CONFIG_PATH="${DATA_ROOT}/Hsap/tuning/cnn_v3/donor/best_config.json"
ACCEPTOR_TUNED_CONFIG_PATH="${DATA_ROOT}/Hsap/tuning/cnn_v3/acceptor/best_config.json"
...
tuned_key_scope() {
	...
	case "${tuned_key}" in
		...
		|block_dilations | residual_channels | pool_every \
		...
		|asym_gamma_pos | asym_gamma_neg | asym_alpha_pos)
			printf '%s\n' "task"
			;;
		# run_model.py keeps cnn_v3 in independent donor/acceptor mode. Pair-only
...
append_tuned_args_for_task() {
	...
	if [[ ! -f "${config_path}" ]]; then
		if [[ "${USE_TUNED_HPARAMS_MODE}" == "required" ]]; then
			echo "[cnn_v3.sh] tuned config path not found: ${config_path}" >&2
			exit 1
		fi
		echo "[cnn_v3.sh] tuned config path not found: ${config_path}; "\
	...
	if ! tuned_output="$(intronmodel_load_tuned_overrides "${config_path}" 2>&1)"; then
		if [[ "${USE_TUNED_HPARAMS_MODE}" == "required" ]]; then
			echo "[cnn_v3.sh] failed to load tuned config: ${config_path}" >&2
			echo "[cnn_v3.sh] detail: ${tuned_output}" >&2
			exit 1
		fi
		echo "[cnn_v3.sh] failed to load tuned config: ${config_path}; "\
			"using CONFIG defaults for species=${species}, target=${task_name}." >&2
		return
	fi
		...
		case "${key_scope}" in
			...
			shared)
				...
				if [[ -n "${existing_value}" && "${existing_value}" != "${tuned_value}" ]]; then
					echo "[cnn_v3.sh] conflicting shared tuned parameter ${tuned_key}: "\
	...
	echo "[cnn_v3.sh] tuned params loaded from ${config_path} "\
...
run_species_once() {
	...
	echo "[cnn_v3.sh] species=${species} mode=independent tasks=donor,acceptor"
...
USE_TUNED_HPARAMS_MODE="$(
	intronmodel_normalize_use_tuned_mode "${USE_TUNED_HPARAMS}" "cnn_v3.sh"
)"
...
if [[ ${#SPECIES_LIST_RESOLVED[@]} -eq 0 ]]; then
	echo "[cnn_v3.sh] SPECIES resolved to an empty list." >&2
	exit 1
fi
if [[ ${#SPECIES_LIST_RESOLVED[@]} -le 1 || ${#GPU_ID_LIST[@]} -le 1 || ${PARALLEL_SLOT_COUNT} -le 1 ]]; then
	...
	echo "[cnn_v3.sh] species-parallel run across GPUs: ${gpu_csv}"
	...
	while [[ ${#pending_species[@]} -gt 0 || ${running_count} -gt 0 ]]; do
		while [[ ${#pending_species[@]} -gt 0 && ${#available_gpu_ids[@]} -gt 0 && ${stop_submitting} -eq 0 ]]; do
			...
			echo "[cnn_v3.sh] species dispatch: ${species} -> gpu=${gpu_id}"
		...
		if [[ -n "${completed_species}" ]]; then
			echo "[cnn_v3.sh] species complete: ${completed_species} gpu=${completed_gpu} exit=${completed_code}"
		fi
