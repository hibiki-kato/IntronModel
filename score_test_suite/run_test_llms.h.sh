...
for d in {rna,cds}-*; do
  [[ -d "${d}" ]] || continue
  perl -ane '$thr=9.95025687741945e-01; $p=$F[1]+0; $score=500*log($p/$thr); $score=$score>-150?$score:-1000; print "$F[0]\t$score\n"' Students/out.gt.$d.h.txt > $d/out.gt.txt && \
  perl -ane '$thr=7.52745392106659e-01; $p=$F[1]+0; $score=500*log($p/$thr); $score=$score>-150?$score:-1000; print "$F[0]\t$score\n"' Students/out.ag.$d.h.txt > $d/out.ag.txt && \
