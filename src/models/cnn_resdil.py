...
class InferRuntimeConfig:
    """Resolved runtime controls for residual-CNN inference."""
...
class _ResDilBlock(nn.Module):
    """Residual dilated 1D convolution block."""
...
class ResDilSpliceCNN(nn.Module):
    """CNN for splice-site scoring with residual dilated blocks."""
    ...
    def __init__(
        ...
        if isinstance(kernel_size, int):
            ...
            elif len(kernel_sizes) < len(channels):
                pad_value = kernel_sizes[-1]
                kernel_sizes.extend([pad_value] * (len(channels) - len(kernel_sizes)))
            elif len(kernel_sizes) > len(channels):
                kernel_sizes = kernel_sizes[: len(channels)]
        ...
        for idx, out_channels in enumerate(channels):
            dilation = dilation_pattern[idx % len(dilation_pattern)]
            stage_kernel_size = kernel_sizes[idx]
            blocks.append(
                _ResDilBlock(
...
def score_sequences(
    ...
    x = torch.from_numpy(np.stack(encoded)).to(
        device=device,
