...
def test_resolve_test_tsv_prefers_processed_over_raw(
    ...
    """Return processed/transcripts.tsv when both processed and raw exist."""
    ...
    (processed_dir / "transcripts.tsv").write_text("processed\n", encoding="utf-8")
    ...
    assert resolved == str(processed_dir / "transcripts.tsv")


def test_resolve_test_tsv_falls_back_to_raw_when_processed_missing(
    tmp_path: Path,
    monkeypatch,
) -> None:
    """Return raw/transcripts.tsv when processed/transcripts.tsv does not exist."""
    base_dir = tmp_path / "Dmel"
    raw_dir = base_dir / "raw"
    raw_dir.mkdir(parents=True)
    (raw_dir / "transcripts.tsv").write_text("raw\n", encoding="utf-8")
    ...
    resolved = data_proc.resolve_test_tsv("Dmel", None)
    assert resolved == str(raw_dir / "transcripts.tsv")
