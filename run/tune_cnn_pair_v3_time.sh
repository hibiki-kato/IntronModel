...
while true; do
	...
	if search_space_resolved="$(
		...
		echo "[tune_cnn_pair_v3_time.sh] using search space: ${search_space_path}"
	else
		search_space_status=$?
		if [[ "${search_space_status}" -eq 2 ]]; then
			exit 1
		fi
		echo "[tune_cnn_pair_v3_time.sh] using embedded pair search space."
	fi
...
JSON
	...
	if [[ "${run_status}" -ne 0 ]]; then
		echo "[tune_cnn_pair_v3_time.sh] cycle=${job_index} failed "\
	...
	printf '[tune_cnn_pair_v3_time.sh] cycle_done=%s cycle_time=%s avg_cycle=%s ' \
		"${job_index}" \
...
echo "[tune_cnn_pair_v3_time.sh] done start=${START_EPOCH} end=${END_EPOCH} "\
