...
if [[ $# -gt 0 ]]; then
	echo "[reservoir.sh] This script is config-only. Edit CONFIG and run without args." >&2
	exit 1
fi
...
print_script_timing() {
	...
	echo "[reservoir.sh] timing: start=${SCRIPT_START_EPOCH} "\
...
resolve_species_case() {
	...
	if [[ ${#matches[@]} -eq 1 ]]; then
		echo "[reservoir.sh] species case normalized: '${raw_species}' -> '${matches[0]}'" >&2
		printf '%s\n' "${matches[0]}"
		return 0
	fi
	if [[ ${#matches[@]} -gt 1 ]]; then
		echo "[reservoir.sh] ambiguous species '${raw_species}'." >&2
		printf '[reservoir.sh] case-insensitive matches: %s\n' "${matches[*]}" >&2
		return 1
	fi
	printf '%s\n' "${raw_species}"
	return 0
...
resolve_tuned_config_path() {
	...
	local task_path="${data_root}/${species}/tuning/reservoir/${task}/best_config.json"
	if [[ -f "${task_path}" ]]; then
		printf '%s\n' "${task_path}"
		return 0
	fi
	...
	local legacy_path="${data_root}/${species}/tuning/reservoir/best_config.json"
...
apply_tuned_overrides_for_task() {
	...
	if [[ ! -f "${config_path}" ]]; then
		echo "[reservoir.sh] tuned ${task} config not found: ${config_path}" >&2
		return 1
	fi
	...
	if ! extracted="$(
		...
		echo "[reservoir.sh] failed to parse tuned ${task} config: ${config_path}" >&2
		echo "[reservoir.sh] parse detail: ${extracted}" >&2
		return 1
	fi
	...
	echo "[reservoir.sh] tuned ${task} loaded from ${config_path} "\
...
if [[ "${USE_COMPILE}" != "off" && "${USE_COMPILE}" != "on" \
	...
	echo "[reservoir.sh] USE_COMPILE must be off|on|auto." >&2
	exit 1
fi
if [[ "${TRAIN_ONLY}" != "0" && "${TRAIN_ONLY}" != "1" ]]; then
	echo "[reservoir.sh] TRAIN_ONLY must be 0 or 1." >&2
	exit 1
fi
if [[ "${SKIP_TRAINING}" != "0" && "${SKIP_TRAINING}" != "1" ]]; then
	echo "[reservoir.sh] SKIP_TRAINING must be 0 or 1." >&2
	exit 1
fi
if [[ "${CONTINUE_TRAINING}" != "0" && "${CONTINUE_TRAINING}" != "1" ]]; then
	echo "[reservoir.sh] CONTINUE_TRAINING must be 0 or 1." >&2
	exit 1
fi
if [[ "${SKIP_TRAINING}" == "1" && "${CONTINUE_TRAINING}" == "1" ]]; then
	echo "[reservoir.sh] CONTINUE_TRAINING=1 cannot be used with SKIP_TRAINING=1." >&2
	exit 1
fi
if [[ "${TRAIN_TARGET}" != "both" && "${TRAIN_TARGET}" != "donor" \
	&& "${TRAIN_TARGET}" != "acceptor" ]]; then
	echo "[reservoir.sh] TRAIN_TARGET must be both|donor|acceptor." >&2
	exit 1
fi
if [[ "${USE_TUNED_HPARAMS}" != "off" && "${USE_TUNED_HPARAMS}" != "auto" \
	&& "${USE_TUNED_HPARAMS}" != "required" ]]; then
	echo "[reservoir.sh] USE_TUNED_HPARAMS must be off|auto|required." >&2
	exit 1
fi
if [[ "${TRAIN_TARGET}" != "both" && "${TRAIN_ONLY}" != "1" ]]; then
	echo "[reservoir.sh] TRAIN_TARGET donor/acceptor requires TRAIN_ONLY=1." >&2
	exit 1
fi
if ! [[ "${MPS_MAX_BATCH_SIZE}" =~ ^[0-9]+$ ]] \
	|| [[ "${MPS_MAX_BATCH_SIZE}" -le 0 ]]; then
	echo "[reservoir.sh] MPS_MAX_BATCH_SIZE must be a positive integer." >&2
	exit 1
fi
...
if [[ "${USE_TUNED_HPARAMS}" != "off" ]]; then
	if [[ -z "${SHARED_TUNED_CONFIG_PATH}" ]]; then
		SHARED_TUNED_CONFIG_PATH="${DATA_ROOT}/${SPECIES}/tuning/reservoir/best_config.json"
	fi
	...
	for tuned_task in "${TUNED_TASKS[@]}"; do
		...
		if resolve_path="$(
			...
			if [[ "${USE_TUNED_HPARAMS}" == "required" ]]; then
				echo "[reservoir.sh] tuned ${tuned_task} config is required but not found." >&2
				exit 1
			fi
			echo "[reservoir.sh] tuned ${tuned_task} config not found; using CONFIG defaults." >&2
			continue
		fi
		...
		if ! apply_tuned_overrides_for_task "${tuned_task}" "${task_config_path}"; then
			...
			echo "[reservoir.sh] tuned ${tuned_task} load failed; using CONFIG defaults." >&2
		fi
	done
fi
...
if [[ -n "${DONOR_INPUT_MODE}" ]]; then
	RUN_ARGS+=(--donor_input_mode "${DONOR_INPUT_MODE}")
fi
if [[ -n "${ACCEPTOR_INPUT_MODE}" ]]; then
	RUN_ARGS+=(--acceptor_input_mode "${ACCEPTOR_INPUT_MODE}")
fi
if [[ -n "${DONOR_KMER_K}" ]]; then
	RUN_ARGS+=(--donor_kmer_k "${DONOR_KMER_K}")
fi
if [[ -n "${ACCEPTOR_KMER_K}" ]]; then
	RUN_ARGS+=(--acceptor_kmer_k "${ACCEPTOR_KMER_K}")
fi
...
if [[ -n "${DONOR_INPUT_DIM}" ]]; then
	RUN_ARGS+=(--donor_input_dim "${DONOR_INPUT_DIM}")
fi
if [[ -n "${ACCEPTOR_INPUT_DIM}" ]]; then
	RUN_ARGS+=(--acceptor_input_dim "${ACCEPTOR_INPUT_DIM}")
fi
if [[ -n "${DONOR_RESERVOIR_SIZE}" ]]; then
	RUN_ARGS+=(--donor_reservoir_size "${DONOR_RESERVOIR_SIZE}")
fi
if [[ -n "${ACCEPTOR_RESERVOIR_SIZE}" ]]; then
	RUN_ARGS+=(--acceptor_reservoir_size "${ACCEPTOR_RESERVOIR_SIZE}")
fi
if [[ -n "${DONOR_SPECTRAL_RADIUS}" ]]; then
	RUN_ARGS+=(--donor_spectral_radius "${DONOR_SPECTRAL_RADIUS}")
fi
if [[ -n "${ACCEPTOR_SPECTRAL_RADIUS}" ]]; then
	RUN_ARGS+=(--acceptor_spectral_radius "${ACCEPTOR_SPECTRAL_RADIUS}")
fi
if [[ -n "${DONOR_LEAK}" ]]; then
	RUN_ARGS+=(--donor_leak "${DONOR_LEAK}")
fi
if [[ -n "${ACCEPTOR_LEAK}" ]]; then
	RUN_ARGS+=(--acceptor_leak "${ACCEPTOR_LEAK}")
fi
if [[ -n "${DONOR_SPARSITY}" ]]; then
	RUN_ARGS+=(--donor_sparsity "${DONOR_SPARSITY}")
fi
if [[ -n "${ACCEPTOR_SPARSITY}" ]]; then
	RUN_ARGS+=(--acceptor_sparsity "${ACCEPTOR_SPARSITY}")
fi
if [[ -n "${DONOR_INPUT_SCALE}" ]]; then
	RUN_ARGS+=(--donor_input_scale "${DONOR_INPUT_SCALE}")
fi
if [[ -n "${ACCEPTOR_INPUT_SCALE}" ]]; then
	RUN_ARGS+=(--acceptor_input_scale "${ACCEPTOR_INPUT_SCALE}")
fi
if [[ -n "${DONOR_POOLING}" ]]; then
	RUN_ARGS+=(--donor_pooling "${DONOR_POOLING}")
fi
if [[ -n "${ACCEPTOR_POOLING}" ]]; then
	RUN_ARGS+=(--acceptor_pooling "${ACCEPTOR_POOLING}")
fi
if [[ -n "${DONOR_READOUT_HIDDEN}" ]]; then
	RUN_ARGS+=(--donor_readout_hidden "${DONOR_READOUT_HIDDEN}")
fi
if [[ -n "${ACCEPTOR_READOUT_HIDDEN}" ]]; then
	RUN_ARGS+=(--acceptor_readout_hidden "${ACCEPTOR_READOUT_HIDDEN}")
fi
if [[ -n "${DONOR_READOUT_DROPOUT}" ]]; then
	RUN_ARGS+=(--donor_readout_dropout "${DONOR_READOUT_DROPOUT}")
fi
if [[ -n "${ACCEPTOR_READOUT_DROPOUT}" ]]; then
	RUN_ARGS+=(--acceptor_readout_dropout "${ACCEPTOR_READOUT_DROPOUT}")
fi
if [[ -n "${DONOR_WASHOUT}" ]]; then
	RUN_ARGS+=(--donor_washout "${DONOR_WASHOUT}")
fi
if [[ -n "${ACCEPTOR_WASHOUT}" ]]; then
	RUN_ARGS+=(--acceptor_washout "${ACCEPTOR_WASHOUT}")
fi
if [[ -n "${DONOR_PREROLL_STEPS}" ]]; then
	RUN_ARGS+=(--donor_preroll_steps "${DONOR_PREROLL_STEPS}")
fi
if [[ -n "${ACCEPTOR_PREROLL_STEPS}" ]]; then
	RUN_ARGS+=(--acceptor_preroll_steps "${ACCEPTOR_PREROLL_STEPS}")
fi
if [[ -n "${DONOR_READ_ORDER}" ]]; then
	RUN_ARGS+=(--donor_read_order "${DONOR_READ_ORDER}")
fi
if [[ -n "${ACCEPTOR_READ_ORDER}" ]]; then
	RUN_ARGS+=(--acceptor_read_order "${ACCEPTOR_READ_ORDER}")
fi
...
echo "[reservoir.sh] Start unified pipeline"
echo "[reservoir.sh] species=${SPECIES} train_only=${TRAIN_ONLY}"
python3 "${PROJECT_ROOT}/src/run_model.py" "${RUN_ARGS[@]}"
echo "[reservoir.sh] Done"
echo "[reservoir.sh] site_score=${OUTPUT_SITE_SCORE_TSV}"
echo "[reservoir.sh] transcript_score=${OUTPUT_TRANS_SCORE_TSV}"
echo "[reservoir.sh] eval_score=${OUTPUT_EVAL_SCORE_TXT}"
