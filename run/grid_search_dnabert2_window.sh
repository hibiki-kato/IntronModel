...
if [[ $# -gt 0 ]]; then
	echo "[grid_search_dnabert2_window.sh] This script is config-only." \
...
resolve_python_bin() {
	intronmodel_resolve_python_bin "grid_search_dnabert2_window.sh"
}
...
resolve_dnabert_model() {
	...
	MODEL_NAME="$(
		intronmodel_resolve_dnabert_model_name \
			"grid_search_dnabert2_window.sh" \
			"${variant}"
	)" || return 1
...
for idx in "${!FILTERED_SPECIES[@]}"; do
	...
	echo "[grid_search_dnabert2_window.sh] species=${sp} target=${TARGET}" \
