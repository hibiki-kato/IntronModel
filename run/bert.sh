...
if [[ $# -gt 0 ]]; then
	echo "[bert.sh] This script is config-only. Edit CONFIG and run without args." >&2
	exit 1
fi
...
resolve_species_case() {
	...
	if [[ ${#matches[@]} -eq 1 ]]; then
		echo "[bert.sh] species case normalized: '${raw_species}' -> '${matches[0]}'" >&2
		printf '%s\n' "${matches[0]}"
		return 0
	fi
	if [[ ${#matches[@]} -gt 1 ]]; then
		echo "[bert.sh] ambiguous species '${raw_species}'." >&2
		printf '[bert.sh] case-insensitive matches: %s\n' "${matches[*]}" >&2
		return 1
	fi
	printf '%s\n' "${raw_species}"
	return 0
...
resolve_tuned_config_path() {
	...
	local task_path="${data_root}/${species}/tuning/bert/${task}/best_config.json"
	if [[ -f "${task_path}" ]]; then
		printf '%s\n' "${task_path}"
		return 0
	fi
	...
	local legacy_path="${data_root}/${species}/tuning/bert/best_config.json"
...
apply_tuned_overrides_for_task() {
	...
	if [[ ! -f "${config_path}" ]]; then
		echo "[bert.sh] tuned ${task} config not found: ${config_path}" >&2
		return 1
	fi
	...
	if ! extracted="$(
		...
		echo "[bert.sh] failed to parse tuned ${task} config: ${config_path}" >&2
		echo "[bert.sh] parse detail: ${extracted}" >&2
		return 1
	fi
	...
	echo "[bert.sh] tuned ${task} loaded from ${config_path} "\
...
if [[ "${USE_COMPILE}" != "off" && "${USE_COMPILE}" != "on" \
	...
	echo "[bert.sh] USE_COMPILE must be off|on|auto." >&2
	exit 1
fi
if [[ "${TRAIN_ONLY}" != "0" && "${TRAIN_ONLY}" != "1" ]]; then
	echo "[bert.sh] TRAIN_ONLY must be 0 or 1." >&2
	exit 1
fi
if [[ "${SKIP_TRAINING}" != "0" && "${SKIP_TRAINING}" != "1" ]]; then
	echo "[bert.sh] SKIP_TRAINING must be 0 or 1." >&2
	exit 1
fi
if [[ "${CONTINUE_TRAINING}" != "0" && "${CONTINUE_TRAINING}" != "1" ]]; then
	echo "[bert.sh] CONTINUE_TRAINING must be 0 or 1." >&2
	exit 1
fi
if [[ "${SKIP_TRAINING}" == "1" && "${CONTINUE_TRAINING}" == "1" ]]; then
	echo "[bert.sh] CONTINUE_TRAINING=1 cannot be used with SKIP_TRAINING=1." >&2
	exit 1
fi
if [[ "${TRAIN_TARGET}" != "both" && "${TRAIN_TARGET}" != "donor" \
	&& "${TRAIN_TARGET}" != "acceptor" ]]; then
	echo "[bert.sh] TRAIN_TARGET must be both|donor|acceptor." >&2
	exit 1
fi
if [[ "${USE_TUNED_HPARAMS}" != "off" && "${USE_TUNED_HPARAMS}" != "auto" \
	&& "${USE_TUNED_HPARAMS}" != "required" ]]; then
	echo "[bert.sh] USE_TUNED_HPARAMS must be off|auto|required." >&2
	exit 1
fi
if [[ "${TRAIN_TARGET}" != "both" && "${TRAIN_ONLY}" != "1" ]]; then
	echo "[bert.sh] TRAIN_TARGET donor/acceptor requires TRAIN_ONLY=1." >&2
	exit 1
fi
if ! [[ "${MPS_MAX_BATCH_SIZE}" =~ ^[0-9]+$ ]] \
	|| [[ "${MPS_MAX_BATCH_SIZE}" -le 0 ]]; then
	echo "[bert.sh] MPS_MAX_BATCH_SIZE must be a positive integer." >&2
	exit 1
fi
...
if [[ "${USE_TUNED_HPARAMS}" != "off" ]]; then
	if [[ -z "${SHARED_TUNED_CONFIG_PATH}" ]]; then
		SHARED_TUNED_CONFIG_PATH="${DATA_ROOT}/${SPECIES}/tuning/bert/best_config.json"
	fi
	...
	for tuned_task in "${TUNED_TASKS[@]}"; do
		...
		if resolve_path="$(
			...
			if [[ "${USE_TUNED_HPARAMS}" == "required" ]]; then
				echo "[bert.sh] tuned ${tuned_task} config is required but not found." >&2
				exit 1
			fi
			echo "[bert.sh] tuned ${tuned_task} config not found; "\
		...
		if ! apply_tuned_overrides_for_task "${tuned_task}" "${task_config_path}"; then
			...
			echo "[bert.sh] tuned ${tuned_task} load failed; "\
				"using CONFIG defaults." >&2
		fi
	done
fi
...
if [[ -n "${DONOR_D_MODEL}" ]]; then
	RUN_ARGS+=(--donor_d_model "${DONOR_D_MODEL}")
fi
if [[ -n "${ACCEPTOR_D_MODEL}" ]]; then
	RUN_ARGS+=(--acceptor_d_model "${ACCEPTOR_D_MODEL}")
fi
if [[ -n "${DONOR_N_HEADS}" ]]; then
	RUN_ARGS+=(--donor_n_heads "${DONOR_N_HEADS}")
fi
if [[ -n "${ACCEPTOR_N_HEADS}" ]]; then
	RUN_ARGS+=(--acceptor_n_heads "${ACCEPTOR_N_HEADS}")
fi
if [[ -n "${DONOR_N_LAYERS}" ]]; then
	RUN_ARGS+=(--donor_n_layers "${DONOR_N_LAYERS}")
fi
if [[ -n "${ACCEPTOR_N_LAYERS}" ]]; then
	RUN_ARGS+=(--acceptor_n_layers "${ACCEPTOR_N_LAYERS}")
fi
if [[ -n "${DONOR_FF_MULT}" ]]; then
	RUN_ARGS+=(--donor_ff_mult "${DONOR_FF_MULT}")
fi
if [[ -n "${ACCEPTOR_FF_MULT}" ]]; then
	RUN_ARGS+=(--acceptor_ff_mult "${ACCEPTOR_FF_MULT}")
fi
...
echo "[bert.sh] Start unified pipeline"
echo "[bert.sh] species=${SPECIES} train_only=${TRAIN_ONLY}"
python3 "${PROJECT_ROOT}/src/run_model.py" "${RUN_ARGS[@]}"
echo "[bert.sh] Done"
echo "[bert.sh] site_score=${OUTPUT_SITE_SCORE_TSV}"
echo "[bert.sh] transcript_score=${OUTPUT_TRANS_SCORE_TSV}"
echo "[bert.sh] eval_score=${OUTPUT_EVAL_SCORE_TXT}"
