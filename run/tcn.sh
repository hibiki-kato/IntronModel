...
if [[ $# -gt 0 ]]; then
	echo "[tcn.sh] This script is config-only. Edit CONFIG and run without args." >&2
	exit 1
fi
...
resolve_species_case() {
	...
	if [[ ${#matches[@]} -eq 1 ]]; then
		echo "[tcn.sh] species case normalized: '${raw_species}' -> '${matches[0]}'" >&2
		printf '%s\n' "${matches[0]}"
		return 0
	fi
	if [[ ${#matches[@]} -gt 1 ]]; then
		echo "[tcn.sh] ambiguous species '${raw_species}'." >&2
		printf '[tcn.sh] case-insensitive matches: %s\n' "${matches[*]}" >&2
		return 1
	fi
	printf '%s\n' "${raw_species}"
	return 0
...
resolve_tuned_config_path() {
	...
	local task_path="${data_root}/${species}/tuning/tcn/${task}/best_config.json"
	if [[ -f "${task_path}" ]]; then
		printf '%s\n' "${task_path}"
		return 0
	fi
	...
	local legacy_path="${data_root}/${species}/tuning/tcn/best_config.json"
...
apply_tuned_overrides_for_task() {
	...
	if [[ ! -f "${config_path}" ]]; then
		echo "[tcn.sh] tuned ${task} config not found: ${config_path}" >&2
		return 1
	fi
	...
	if ! extracted="$(
		...
		echo "[tcn.sh] failed to parse tuned ${task} config: ${config_path}" >&2
		echo "[tcn.sh] parse detail: ${extracted}" >&2
		return 1
	fi
	...
	echo "[tcn.sh] tuned ${task} loaded from ${config_path} "\
...
if [[ "${USE_COMPILE}" != "off" && "${USE_COMPILE}" != "on" \
	...
	echo "[tcn.sh] USE_COMPILE must be off|on|auto." >&2
	exit 1
fi
if [[ "${TRAIN_ONLY}" != "0" && "${TRAIN_ONLY}" != "1" ]]; then
	echo "[tcn.sh] TRAIN_ONLY must be 0 or 1." >&2
	exit 1
fi
if [[ "${SKIP_TRAINING}" != "0" && "${SKIP_TRAINING}" != "1" ]]; then
	echo "[tcn.sh] SKIP_TRAINING must be 0 or 1." >&2
	exit 1
fi
if [[ "${CONTINUE_TRAINING}" != "0" && "${CONTINUE_TRAINING}" != "1" ]]; then
	echo "[tcn.sh] CONTINUE_TRAINING must be 0 or 1." >&2
	exit 1
fi
if [[ "${SKIP_TRAINING}" == "1" && "${CONTINUE_TRAINING}" == "1" ]]; then
	echo "[tcn.sh] CONTINUE_TRAINING=1 cannot be used with SKIP_TRAINING=1." >&2
	exit 1
fi
if [[ "${TRAIN_TARGET}" != "both" && "${TRAIN_TARGET}" != "donor" \
	&& "${TRAIN_TARGET}" != "acceptor" ]]; then
	echo "[tcn.sh] TRAIN_TARGET must be both|donor|acceptor." >&2
	exit 1
fi
if [[ "${USE_TUNED_HPARAMS}" != "off" && "${USE_TUNED_HPARAMS}" != "auto" \
	&& "${USE_TUNED_HPARAMS}" != "required" ]]; then
	echo "[tcn.sh] USE_TUNED_HPARAMS must be off|auto|required." >&2
	exit 1
fi
if [[ "${TRAIN_TARGET}" != "both" && "${TRAIN_ONLY}" != "1" ]]; then
	echo "[tcn.sh] TRAIN_TARGET donor/acceptor requires TRAIN_ONLY=1." >&2
	exit 1
fi
if ! [[ "${MPS_MAX_BATCH_SIZE}" =~ ^[0-9]+$ ]] || [[ "${MPS_MAX_BATCH_SIZE}" -le 0 ]]; then
	echo "[tcn.sh] MPS_MAX_BATCH_SIZE must be a positive integer." >&2
	exit 1
fi
...
if [[ "${USE_TUNED_HPARAMS}" != "off" ]]; then
	if [[ -z "${SHARED_TUNED_CONFIG_PATH}" ]]; then
		SHARED_TUNED_CONFIG_PATH="${DATA_ROOT}/${SPECIES}/tuning/tcn/best_config.json"
	fi
	...
	for tuned_task in "${TUNED_TASKS[@]}"; do
		...
		if resolve_path="$(
			...
			if [[ "${USE_TUNED_HPARAMS}" == "required" ]]; then
				echo "[tcn.sh] tuned ${tuned_task} config is required but not found." >&2
				exit 1
			fi
			echo "[tcn.sh] tuned ${tuned_task} config not found; "\
		...
		if ! apply_tuned_overrides_for_task "${tuned_task}" "${task_config_path}"; then
			...
			echo "[tcn.sh] tuned ${tuned_task} load failed; "\
				"using CONFIG defaults." >&2
		fi
	done
fi
...
params = {
    ...
    "tcn_block_repeats": int("${TCN_BLOCK_REPEATS}"),
    "tcn_causal": int("${TCN_CAUSAL}"),
...
if [[ -n "${DONOR_TCN_BLOCK_REPEATS}" ]]; then
	RUN_ARGS+=(--donor_tcn_block_repeats "${DONOR_TCN_BLOCK_REPEATS}")
fi
if [[ -n "${ACCEPTOR_TCN_BLOCK_REPEATS}" ]]; then
	RUN_ARGS+=(--acceptor_tcn_block_repeats "${ACCEPTOR_TCN_BLOCK_REPEATS}")
fi
if [[ -n "${DONOR_TCN_CAUSAL}" ]]; then
	RUN_ARGS+=(--donor_tcn_causal "${DONOR_TCN_CAUSAL}")
fi
if [[ -n "${ACCEPTOR_TCN_CAUSAL}" ]]; then
	RUN_ARGS+=(--acceptor_tcn_causal "${ACCEPTOR_TCN_CAUSAL}")
fi
...
echo "[tcn.sh] Start unified pipeline"
echo "[tcn.sh] species=${SPECIES} perf_mode=${PERF_MODE} train_only=${TRAIN_ONLY}"
python3 "${PROJECT_ROOT}/src/run_model.py" "${RUN_ARGS[@]}"
echo "[tcn.sh] Done"
echo "[tcn.sh] site_score=${OUTPUT_SITE_SCORE_TSV}"
echo "[tcn.sh] transcript_score=${OUTPUT_TRANS_SCORE_TSV}"
echo "[tcn.sh] eval_score=${OUTPUT_EVAL_SCORE_TXT}"
