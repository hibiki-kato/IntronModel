...
Options:
  ...
  --mix-mode <mode>               both | donor_pos | acceptor_pos
                                  (default: both)
  --samples-per-positive <int>    Generated rows per positive per enabled side
                                  (default: 1)
...
echo "[make_mixed_pair_neg_data.sh] samples_per_positive=${SAMPLES_PER_POSITIVE}"
...
args=(
	...
	--mix-mode "${MIX_MODE}"
	--samples-per-positive "${SAMPLES_PER_POSITIVE}"
	--seed "${SEED}"
)
