...
if [[ $# -gt 0 ]]; then
	echo "[tune_reservoir.sh] This script is config-only. Edit top CONFIG and run without args." >&2
	exit 1
fi
...
print_script_timing() {
	...
	echo "[tune_reservoir.sh] timing: start=${SCRIPT_START_EPOCH} "\
...
resolve_species_case() {
	...
	if [[ ${#matches[@]} -eq 1 ]]; then
		echo "[tune_reservoir.sh] species case normalized: '${raw_species}' -> '${matches[0]}'" >&2
		printf '%s\n' "${matches[0]}"
		return 0
	fi
	if [[ ${#matches[@]} -gt 1 ]]; then
		echo "[tune_reservoir.sh] ambiguous species '${raw_species}'." >&2
		printf '[tune_reservoir.sh] case-insensitive matches: %s\n' "${matches[*]}" >&2
		return 1
	fi
	printf '%s\n' "${raw_species}"
	return 0
...
resolve_tune_targets() {
	...
	for part in "${parts[@]}"; do
		...
		if [[ "${target}" != "donor" && "${target}" != "acceptor" ]]; then
			echo "[tune_reservoir.sh] invalid target: ${target}" >&2
			echo "[tune_reservoir.sh] TUNE_TARGETS must contain donor and/or acceptor." >&2
			return 1
		fi
		resolved+=("${target}")
	done
	...
	if [[ ${#resolved[@]} -eq 0 ]]; then
		echo "[tune_reservoir.sh] no valid tuning targets configured." >&2
		return 1
	fi
...
resolve_python_bin() {
	...
	echo "[tune_reservoir.sh] python interpreter not found (python3/python)." >&2
	return 1
}
...
resolve_search_space_file() {
	...
	if [[ -n "${explicit_file}" && "${explicit_file}" != "auto" ]]; then
		...
		echo "[tune_reservoir.sh] SEARCH_SPACE_FILE not found: ${explicit_file}" >&2
		return 2
	fi

	local target_file="${DATA_ROOT}/${species}/tuning/reservoir/${target}/search_space.json"
	if [[ -f "${target_file}" ]]; then
		printf '%s\n' "${target_file}"
		return 0
	fi

	local species_file="${DATA_ROOT}/${species}/tuning/reservoir/search_space.json"
...
target_root = data_root / species / "tuning" / "reservoir" / target
...
if [[ "${QUICK_TRIALS_MODE}" != "fixed" && "${QUICK_TRIALS_MODE}" != "budget" ]]; then
	echo "[tune_reservoir.sh] QUICK_TRIALS_MODE must be fixed|budget." >&2
	exit 1
fi
if [[ "${SEARCH_ALGO}" != "random" && "${SEARCH_ALGO}" != "history_guided" ]]; then
	echo "[tune_reservoir.sh] SEARCH_ALGO must be random|history_guided." >&2
	exit 1
fi
if [[ "${QUICK_COMPILE_MODE}" != "off" && "${QUICK_COMPILE_MODE}" != "on" \
	&& "${QUICK_COMPILE_MODE}" != "auto" ]]; then
	echo "[tune_reservoir.sh] QUICK_COMPILE_MODE must be off|on|auto." >&2
	exit 1
fi
if [[ "${FULL_COMPILE_MODE}" != "off" && "${FULL_COMPILE_MODE}" != "on" \
	&& "${FULL_COMPILE_MODE}" != "auto" ]]; then
	echo "[tune_reservoir.sh] FULL_COMPILE_MODE must be off|on|auto." >&2
	exit 1
fi
if ! [[ "${QUICK_TRIALS}" =~ ^[0-9]+$ ]] || [[ "${QUICK_TRIALS}" -le 0 ]]; then
	echo "[tune_reservoir.sh] QUICK_TRIALS must be a positive integer." >&2
	exit 1
fi
if ! [[ "${HISTORY_TOP_N}" =~ ^[0-9]+$ ]] || [[ "${HISTORY_TOP_N}" -le 0 ]]; then
	echo "[tune_reservoir.sh] HISTORY_TOP_N must be a positive integer." >&2
	exit 1
fi
if ! [[ "${MIN_QUICK_TRIALS}" =~ ^[0-9]+$ ]] || [[ "${MIN_QUICK_TRIALS}" -le 0 ]]; then
	echo "[tune_reservoir.sh] MIN_QUICK_TRIALS must be a positive integer." >&2
	exit 1
fi
if ! [[ "${TARGET_TIME_BUDGET_MINUTES}" =~ ^[0-9]+$ ]]; then
	echo "[tune_reservoir.sh] TARGET_TIME_BUDGET_MINUTES must be an integer." >&2
	exit 1
fi
if ! [[ "${TOTAL_TIME_BUDGET_MINUTES}" =~ ^[0-9]+$ ]]; then
	echo "[tune_reservoir.sh] TOTAL_TIME_BUDGET_MINUTES must be an integer." >&2
	exit 1
fi
if ! [[ "${QUICK_TRIAL_SEC_FALLBACK}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_reservoir.sh] QUICK_TRIAL_SEC_FALLBACK must be a positive number." >&2
	exit 1
fi
if ! [[ "${GUIDED_RANDOM_FRACTION}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_reservoir.sh] GUIDED_RANDOM_FRACTION must be numeric in [0,1]." >&2
	exit 1
fi
if ! awk -v x="${GUIDED_RANDOM_FRACTION}" 'BEGIN{exit !(x>=0 && x<=1)}'; then
	echo "[tune_reservoir.sh] GUIDED_RANDOM_FRACTION must be in [0,1]." >&2
	exit 1
fi
if ! [[ "${GUIDED_MUTATION_RATE}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_reservoir.sh] GUIDED_MUTATION_RATE must be numeric in [0,1]." >&2
	exit 1
fi
if ! awk -v x="${GUIDED_MUTATION_RATE}" 'BEGIN{exit !(x>=0 && x<=1)}'; then
	echo "[tune_reservoir.sh] GUIDED_MUTATION_RATE must be in [0,1]." >&2
	exit 1
fi
if [[ "${QUICK_TRIALS_MODE}" == "budget" ]]; then
	if [[ "${TARGET_TIME_BUDGET_MINUTES}" -le 0 && "${TOTAL_TIME_BUDGET_MINUTES}" -le 0 ]]; then
		echo "[tune_reservoir.sh] budget mode requires TARGET_TIME_BUDGET_MINUTES>0 "\
...
echo "[tune_reservoir.sh] species=${SPECIES}"
echo "[tune_reservoir.sh] targets=${TARGET_LIST[*]}"

for TARGET in "${TARGET_LIST[@]}"; do
	OBJECTIVE_METRIC="${TARGET}_pr_auc"
	OUTPUT_DIR="${DATA_ROOT}/${SPECIES}/tuning/reservoir/${TARGET}/${RUN_TIMESTAMP}"
	GLOBAL_BEST_CONFIG_PATH="${DATA_ROOT}/${SPECIES}/tuning/reservoir/${TARGET}/best_config.json"
	...
	if [[ "${QUICK_TRIALS_MODE}" == "budget" ]]; then
		...
		if [[ "${TARGET_BUDGET_MINUTES}" -le 0 ]]; then
			echo "[tune_reservoir.sh] target budget computed as 0 minutes. "\
		...
		if ! estimate_result="$(
			...
			echo "[tune_reservoir.sh] failed to estimate quick trials from budget." >&2
			echo "[tune_reservoir.sh] estimator detail: ${estimate_result}" >&2
			exit 1
		fi
		IFS=$'\t' read -r \
			QUICK_TRIALS_TARGET \
		...
		echo "[tune_reservoir.sh] target=${TARGET} budget=${TARGET_BUDGET_MINUTES}min "\
	...
	if search_space_resolved="$(
		...
		if ! target_space_json="$(
			...
			echo "[tune_reservoir.sh] failed to parse search-space file: ${search_space_path}" >&2
			echo "[tune_reservoir.sh] parse detail: ${target_space_json}" >&2
			exit 1
		fi
		TARGET_SEARCH_SPACE_JSON="${target_space_json}"
		echo "[tune_reservoir.sh] target=${TARGET} search_space_file=${search_space_path}"
	else
		search_space_status=$?
		if [[ "${search_space_status}" -eq 2 ]]; then
			exit 1
		fi
		echo "[tune_reservoir.sh] target=${TARGET} search_space_file=<embedded_${TARGET}>"
	fi
...
JSON
	...
	if ! "${PYTHON_BIN}" "${PROJECT_ROOT}/src/tools/hparam_search.py" \
		...
		echo "[tune_reservoir.sh] target=${TARGET} failed start=${TARGET_START_EPOCH} "\
	...
	target_elapsed_seconds=$((SECONDS - TARGET_START_SECONDS))
	target_elapsed_hms="$(format_elapsed "${target_elapsed_seconds}")"
	echo "[tune_reservoir.sh] target=${TARGET} done start=${TARGET_START_EPOCH} "\
