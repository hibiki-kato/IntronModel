...
DEFAULT_SEARCH_SPACE_JSON_SITE="$(cat <<'JSON'
{
  ...
  "weight_decay": {"type": "float", "min": 1e-8, "max": 2e-2, "scale": "log"},
	...
	"arch_max_depth": {"type": "int", "min": 3, "max": 8, "step": 1},
	"arch_init_channels": {
		"type": "categorical",
	...
	"arch_mutation_steps": {"type": "int", "min": 1, "max": 4, "step": 1},
	"arch_max_dilation": {
		"type": "categorical",
		"values": [4, 8, 16]
	},
	"arch_add_block_prob": {"type": "float", "min": 0.1, "max": 0.8, "scale": "linear"},
	"arch_widen_prob": {"type": "float", "min": 0.1, "max": 0.9, "scale": "linear"},
	"arch_dilation_prob": {"type": "float", "min": 0.1, "max": 0.9, "scale": "linear"},
	"arch_residual_prob": {"type": "float", "min": 0.1, "max": 0.9, "scale": "linear"},
	"max_pool_size": {"type": "categorical", "values": [1, 2, 3, 4]},
	"pool_every": {"type": "categorical", "values": [1, 2, 3]},
...
run_cycle_process() {
	...
	local stdout_log="$7"
	local run_status=0
	local cycle_prefix="[tune_cnn_v3_time.sh][cycle=${cycle_index}][species=${species}][target=${target_name}][seed=${base_seed}]"
	...
	if [[ "${run_status}" -eq 124 ]]; then
		{
			echo "[tune_cnn_v3_time.sh] time budget reached; "\
	...
	if [[ "${run_status}" -ne 0 ]]; then
		echo "[tune_cnn_v3_time.sh] cycle=${cycle_index} failed "\
	...
	cat > "${config_path}" <<JSON
{
  ...
  "max_oom_retries": ${MAX_OOM_RETRIES},
    ...
    "val_frac": ${VAL_FRAC},
    "validation_metric": "${OBJECTIVE_METRIC}",
...
payload = {
    ...
    "seed": int(sys.argv[4]),
    "tuning_model_name": sys.argv[5],
    "template_config_path": sys.argv[6],
    "output_parent_dir": sys.argv[7],
    "plot_target_name": sys.argv[8],
}
...
if [[ "${TRIAL_STREAM_MODE}" != "auto" \
	...
	echo "[tune_cnn_v3_time.sh] TRIAL_STREAM_MODE must be auto|full|errors|silent." >&2
	exit 1
fi
if [[ "${ENABLE_PHASE_OVERLAP}" != "0" \
	&& "${ENABLE_PHASE_OVERLAP}" != "1" ]]; then
	echo "[tune_cnn_v3_time.sh] ENABLE_PHASE_OVERLAP must be 0 or 1." >&2
	exit 1
fi
if [[ "${SEARCH_ALGO}" != "random" \
	&& "${SEARCH_ALGO}" != "history_guided" \
	&& "${SEARCH_ALGO}" != "reinforce" ]]; then
	echo "[tune_cnn_v3_time.sh] SEARCH_ALGO must be random|history_guided|reinforce." >&2
	exit 1
fi
...
if ! [[ "${REINFORCE_TEMPERATURE}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_cnn_v3_time.sh] REINFORCE_TEMPERATURE must be numeric > 0." >&2
	exit 1
fi
if ! awk -v x="${REINFORCE_TEMPERATURE}" 'BEGIN{exit !(x>0)}'; then
	echo "[tune_cnn_v3_time.sh] REINFORCE_TEMPERATURE must be > 0." >&2
	exit 1
fi
...
if [[ ${#TARGET_ORDER[@]} -eq 0 ]]; then
	echo "[tune_cnn_v3_time.sh] TARGET_ORDER must contain at least one task." >&2
	exit 1
fi
for target_name in "${TARGET_ORDER[@]}"; do
	if [[ "${target_name}" != "donor" && "${target_name}" != "acceptor" ]]; then
		echo "[tune_cnn_v3_time.sh] TARGET_ORDER values must be donor|acceptor." >&2
		exit 1
	fi
done
...
if [[ "${OBJECTIVE_METRIC}" != "pr_auc" \
	...
	echo "[tune_cnn_v3_time.sh] OBJECTIVE_METRIC must be pr_auc|max_f1." >&2
	exit 1
fi
...
echo "[tune_cnn_v3_time.sh] targets=${TARGET_ORDER[*]}"
...
for base_seed in "${SEED_VALUES[@]}"; do
	for raw_species in "${JOB_ORDER[@]}"; do
		...
		for target_name in "${TARGET_ORDER[@]}"; do
			...
			template_path="${scheduler_root}/${species}_${target_name}_seed${base_seed}.template.json"
			output_parent_dir="${DATA_ROOT}/${species}/tuning/${TUNING_MODEL_NAME}/${target_name}"
			write_cycle_template_config \
				"${template_path}" \
				"${species}" \
				"${target_name}" \
