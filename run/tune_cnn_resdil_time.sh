...
if [[ $# -gt 0 ]]; then
	echo "[tune_cnn_resdil_time.sh] This script is config-only." \
...
resolve_search_space_file() {
	...
	if [[ -n "${explicit_file}" && "${explicit_file}" != "auto" ]]; then
		...
		echo "[tune_cnn_resdil_time.sh] SEARCH_SPACE_FILE not found: ${explicit_file}" >&2
		return 2
	fi
...
run_double_descent_plot() {
	...
	"${python_bin}" "${project_root}/src/tools/plot_tuning_double_descent.py" \
		...
		--target "${target_name}" \
		--model "cnn_resdil" || true
}

if ! [[ "${TIME_BUDGET_MINUTES}" =~ ^[0-9]+$ ]] \
	|| [[ "${TIME_BUDGET_MINUTES}" -le 0 ]]; then
	echo "[tune_cnn_resdil_time.sh] TIME_BUDGET_MINUTES must be a positive integer." >&2
	exit 1
fi
if ! [[ "${QUICK_TRIALS}" =~ ^[0-9]+$ ]] || [[ "${QUICK_TRIALS}" -le 0 ]]; then
	echo "[tune_cnn_resdil_time.sh] QUICK_TRIALS must be a positive integer." >&2
	exit 1
fi
if ! [[ "${QUICK_EPOCHS}" =~ ^[0-9]+$ ]] || [[ "${QUICK_EPOCHS}" -le 0 ]]; then
	echo "[tune_cnn_resdil_time.sh] QUICK_EPOCHS must be a positive integer." >&2
	exit 1
fi
if ! [[ "${TOP_K}" =~ ^[0-9]+$ ]] || [[ "${TOP_K}" -le 0 ]]; then
	echo "[tune_cnn_resdil_time.sh] TOP_K must be a positive integer." >&2
	exit 1
fi
if ! [[ "${FULL_EPOCHS}" =~ ^[0-9]+$ ]] || [[ "${FULL_EPOCHS}" -le 0 ]]; then
	echo "[tune_cnn_resdil_time.sh] FULL_EPOCHS must be a positive integer." >&2
	exit 1
fi
if [[ "${QUICK_COMPILE_MODE}" != "off" \
	&& "${QUICK_COMPILE_MODE}" != "on" \
	&& "${QUICK_COMPILE_MODE}" != "auto" ]]; then
	echo "[tune_cnn_resdil_time.sh] QUICK_COMPILE_MODE must be off|on|auto." >&2
	exit 1
fi
if [[ "${FULL_COMPILE_MODE}" != "off" \
	&& "${FULL_COMPILE_MODE}" != "on" \
	&& "${FULL_COMPILE_MODE}" != "auto" ]]; then
	echo "[tune_cnn_resdil_time.sh] FULL_COMPILE_MODE must be off|on|auto." >&2
	exit 1
fi
if [[ "${SEARCH_ALGO}" != "random" && "${SEARCH_ALGO}" != "history_guided" ]]; then
	echo "[tune_cnn_resdil_time.sh] SEARCH_ALGO must be random|history_guided." >&2
	exit 1
fi
if ! [[ "${HISTORY_TOP_N}" =~ ^[0-9]+$ ]] || [[ "${HISTORY_TOP_N}" -le 0 ]]; then
	echo "[tune_cnn_resdil_time.sh] HISTORY_TOP_N must be a positive integer." >&2
	exit 1
fi
if ! [[ "${GUIDED_RANDOM_FRACTION}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_cnn_resdil_time.sh] GUIDED_RANDOM_FRACTION must be numeric in [0,1]." >&2
	exit 1
fi
if ! awk -v x="${GUIDED_RANDOM_FRACTION}" 'BEGIN{exit !(x>=0 && x<=1)}'; then
	echo "[tune_cnn_resdil_time.sh] GUIDED_RANDOM_FRACTION must be in [0,1]." >&2
	exit 1
fi
if ! [[ "${GUIDED_MUTATION_RATE}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_cnn_resdil_time.sh] GUIDED_MUTATION_RATE must be numeric in [0,1]." >&2
	exit 1
fi
if ! awk -v x="${GUIDED_MUTATION_RATE}" 'BEGIN{exit !(x>=0 && x<=1)}'; then
	echo "[tune_cnn_resdil_time.sh] GUIDED_MUTATION_RATE must be in [0,1]." >&2
	exit 1
fi
if [[ ${#JOB_ORDER[@]} -eq 0 ]]; then
	echo "[tune_cnn_resdil_time.sh] JOB_ORDER must contain at least one" \
		"species:target pair." >&2
	exit 1
fi
if [[ "${UPDATE_DOUBLE_DESCENT_PLOT}" != "0" \
	&& "${UPDATE_DOUBLE_DESCENT_PLOT}" != "1" ]]; then
	echo "[tune_cnn_resdil_time.sh] UPDATE_DOUBLE_DESCENT_PLOT must be 0 or 1." >&2
	exit 1
fi
...
echo "[tune_cnn_resdil_time.sh] start=${START_EPOCH} budget=${TIME_BUDGET_MINUTES}min"
echo "[tune_cnn_resdil_time.sh] quick+full cycles: "\
	"quick_trials=${QUICK_TRIALS} quick_epochs=${QUICK_EPOCHS} "\
	"top_k=${TOP_K} full_epochs=${FULL_EPOCHS}"
echo "[tune_cnn_resdil_time.sh] schedule=${JOB_ORDER[*]}"
...
while true; do
	...
	if [[ "${COMPLETED_CYCLES}" -gt 0 ]]; then
		...
		if [[ "${avg_cycle_seconds_guard}" -gt 0 ]] \
			...
			echo "[tune_cnn_resdil_time.sh] stop before next cycle: "\
	...
	if [[ "${target}" != "donor" && "${target}" != "acceptor" ]]; then
		echo "[tune_cnn_resdil_time.sh] invalid target in JOB_ORDER: ${pair}" >&2
		exit 1
	fi

	run_stamp="$(date +%Y%m%d_%H%M%S)"
	run_id="${run_stamp}_c$(printf '%03d' "${job_index}")"
	output_dir="${DATA_ROOT}/${species}/tuning/cnn_resdil/${target}/${run_id}"
	global_best_path="${DATA_ROOT}/${species}/tuning/cnn_resdil/${target}"\
"/best_config.json"
	...
	if [[ -n "${SEED_BEST_CONFIG_PATH}" ]]; then
		...
		if search_space_resolved="$(
			...
			if ! target_space_json="$(
				...
				echo "[tune_cnn_resdil_time.sh] failed to parse search-space file: "\
					"${search_space_path}" >&2
				echo "[tune_cnn_resdil_time.sh] parse detail: ${target_space_json}" >&2
				exit 1
			fi
			target_search_space_json="${target_space_json}"
			echo "[tune_cnn_resdil_time.sh] using search space: ${search_space_path}"
		else
			search_space_status=$?
			if [[ "${search_space_status}" -eq 2 ]]; then
				exit 1
			fi
			echo "[tune_cnn_resdil_time.sh] using embedded ${target} search space."
		fi
...
JSON
	...
	printf '[tune_cnn_resdil_time.sh] cycle=%s elapsed=%s start=%s ' \
	...
	printf '[tune_cnn_resdil_time.sh] cycle_done=%s cycle_time=%s avg_cycle=%s ' \
		"${job_index}" \
...
echo "[tune_cnn_resdil_time.sh] done start=${START_EPOCH} end=${END_EPOCH} "\
