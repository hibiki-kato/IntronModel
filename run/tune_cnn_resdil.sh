...
if [[ $# -gt 0 ]]; then
	echo "[tune_cnn_resdil.sh] This script is config-only. Edit top CONFIG and run without args." >&2
	exit 1
fi
...
print_script_timing() {
	...
	echo "[tune_cnn_resdil.sh] timing: start=${SCRIPT_START_EPOCH} "\
...
resolve_species_case() {
	...
	if [[ ${#matches[@]} -eq 1 ]]; then
		echo "[tune_cnn_resdil.sh] species case normalized: '${raw_species}' -> '${matches[0]}'" >&2
		printf '%s\n' "${matches[0]}"
		return 0
	fi
	if [[ ${#matches[@]} -gt 1 ]]; then
		echo "[tune_cnn_resdil.sh] ambiguous species '${raw_species}'." >&2
		printf '[tune_cnn_resdil.sh] case-insensitive matches: %s\n' "${matches[*]}" >&2
		return 1
	fi
	printf '%s\n' "${raw_species}"
	return 0
...
resolve_tune_targets() {
	...
	for part in "${parts[@]}"; do
		...
		if [[ "${target}" != "donor" && "${target}" != "acceptor" ]]; then
			echo "[tune_cnn_resdil.sh] invalid target: ${target}" >&2
			echo "[tune_cnn_resdil.sh] TUNE_TARGETS must contain donor and/or acceptor." >&2
			return 1
		fi
		resolved+=("${target}")
	done
	...
	if [[ ${#resolved[@]} -eq 0 ]]; then
		echo "[tune_cnn_resdil.sh] no valid tuning targets configured." >&2
		return 1
	fi
...
resolve_python_bin() {
	...
	echo "[tune_cnn_resdil.sh] python interpreter not found (python3/python)." >&2
	return 1
}
...
resolve_search_space_file() {
	...
	if [[ -n "${explicit_file}" && "${explicit_file}" != "auto" ]]; then
		...
		echo "[tune_cnn_resdil.sh] SEARCH_SPACE_FILE not found: ${explicit_file}" >&2
		return 2
	fi
...
if [[ "${QUICK_TRIALS_MODE}" != "fixed" && "${QUICK_TRIALS_MODE}" != "budget" ]]; then
	echo "[tune_cnn_resdil.sh] QUICK_TRIALS_MODE must be fixed|budget." >&2
	exit 1
fi
...
if ! [[ "${QUICK_TRIALS}" =~ ^[0-9]+$ ]] || [[ "${QUICK_TRIALS}" -le 0 ]]; then
	echo "[tune_cnn_resdil.sh] QUICK_TRIALS must be a positive integer." >&2
	exit 1
fi
if ! [[ "${HISTORY_TOP_N}" =~ ^[0-9]+$ ]] || [[ "${HISTORY_TOP_N}" -le 0 ]]; then
	echo "[tune_cnn_resdil.sh] HISTORY_TOP_N must be a positive integer." >&2
	exit 1
fi
if ! [[ "${MIN_QUICK_TRIALS}" =~ ^[0-9]+$ ]] || [[ "${MIN_QUICK_TRIALS}" -le 0 ]]; then
	echo "[tune_cnn_resdil.sh] MIN_QUICK_TRIALS must be a positive integer." >&2
	exit 1
fi
if ! [[ "${TARGET_TIME_BUDGET_MINUTES}" =~ ^[0-9]+$ ]]; then
	echo "[tune_cnn_resdil.sh] TARGET_TIME_BUDGET_MINUTES must be an integer." >&2
	exit 1
fi
if ! [[ "${TOTAL_TIME_BUDGET_MINUTES}" =~ ^[0-9]+$ ]]; then
	echo "[tune_cnn_resdil.sh] TOTAL_TIME_BUDGET_MINUTES must be an integer." >&2
	exit 1
fi
if ! [[ "${QUICK_TRIAL_SEC_FALLBACK}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_cnn_resdil.sh] QUICK_TRIAL_SEC_FALLBACK must be a positive number." >&2
	exit 1
fi
if ! [[ "${GUIDED_RANDOM_FRACTION}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_cnn_resdil.sh] GUIDED_RANDOM_FRACTION must be numeric in [0,1]." >&2
	exit 1
fi
if ! awk -v x="${GUIDED_RANDOM_FRACTION}" 'BEGIN{exit !(x>=0 && x<=1)}'; then
	echo "[tune_cnn_resdil.sh] GUIDED_RANDOM_FRACTION must be in [0,1]." >&2
	exit 1
fi
if ! [[ "${GUIDED_MUTATION_RATE}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_cnn_resdil.sh] GUIDED_MUTATION_RATE must be numeric in [0,1]." >&2
	exit 1
fi
if ! awk -v x="${GUIDED_MUTATION_RATE}" 'BEGIN{exit !(x>=0 && x<=1)}'; then
	echo "[tune_cnn_resdil.sh] GUIDED_MUTATION_RATE must be in [0,1]." >&2
	exit 1
fi
if [[ "${QUICK_TRIALS_MODE}" == "budget" ]]; then
	if [[ "${TARGET_TIME_BUDGET_MINUTES}" -le 0 && "${TOTAL_TIME_BUDGET_MINUTES}" -le 0 ]]; then
		echo "[tune_cnn_resdil.sh] budget mode requires TARGET_TIME_BUDGET_MINUTES>0 "\
...
for TARGET in "${TARGET_LIST[@]}"; do
	...
	if [[ "${QUICK_TRIALS_MODE}" == "budget" ]]; then
		...
		if [[ "${TARGET_BUDGET_MINUTES}" -le 0 ]]; then
			echo "[tune_cnn_resdil.sh] target budget computed as 0 minutes. "\
		...
		if ! estimate_result="$(
			...
			echo "[tune_cnn_resdil.sh] failed to estimate quick trials from budget." >&2
			echo "[tune_cnn_resdil.sh] estimator detail: ${estimate_result}" >&2
			exit 1
		fi
		IFS=$'\t' read -r \
			QUICK_TRIALS_TARGET \
		...
		echo "[tune_cnn_resdil.sh] target=${TARGET} budget=${TARGET_BUDGET_MINUTES}min "\
	...
	if search_space_resolved="$(
		...
		if ! target_space_json="$(
			...
			echo "[tune_cnn_resdil.sh] failed to parse search-space file: ${search_space_path}" >&2
			echo "[tune_cnn_resdil.sh] parse detail: ${target_space_json}" >&2
			exit 1
		fi
