...
if [[ $# -gt 0 ]]; then
	echo "[tune_dnabert_pair.sh] This script is config-only. Edit top CONFIG and run without args." >&2
	exit 1
fi
...
intronmodel_start_timer "tune_dnabert_pair.sh"
trap 'intronmodel_print_timing' EXIT

resolve_species_case() {
	intronmodel_resolve_species_case "$1" "$2" "tune_dnabert_pair.sh"
}
...
resolve_tune_targets() {
	...
	for part in "${parts[@]}"; do
		...
		if [[ "${target}" != "pair" ]]; then
			echo "[tune_dnabert_pair.sh] invalid target: ${target}" >&2
			echo "[tune_dnabert_pair.sh] TUNE_TARGETS must contain pair only." >&2
			return 1
		fi
		resolved+=("${target}")
	done
	...
	if [[ ${#resolved[@]} -eq 0 ]]; then
		echo "[tune_dnabert_pair.sh] no valid tuning targets configured." >&2
		return 1
	fi
	printf '%s\n' "${resolved[@]}"
}
...
resolve_python_bin() {
	intronmodel_resolve_python_bin "tune_dnabert_pair.sh"
}
...
resolve_dnabert_model() {
	...
	if [[ "${normalized_variant}" != "2" \
		...
		echo "[tune_dnabert_pair.sh] DNABERT_VARIANT must be 2, 6, or s." >&2
		return 1
	fi
	...
	if [[ -z "${relative_path}" ]]; then
		echo "[tune_dnabert_pair.sh] pretrained relative path is empty for variant=${variant}." >&2
		return 1
	fi
	...
	if [[ -z "${normalized_relative_path}" ]]; then
		echo "[tune_dnabert_pair.sh] pretrained relative path resolved to empty value." >&2
		return 1
	fi
...
resolve_search_space_file() {
	...
	if [[ -n "${explicit_file}" && "${explicit_file}" != "auto" ]]; then
		...
		echo "[tune_dnabert_pair.sh] SEARCH_SPACE_FILE not found: ${explicit_file}" >&2
		return 2
	fi
...
if [[ "${TRUST_REMOTE_CODE}" != "0" && "${TRUST_REMOTE_CODE}" != "1" ]]; then
	echo "[tune_dnabert_pair.sh] TRUST_REMOTE_CODE must be 0 or 1." >&2
	exit 1
fi

if [[ "${QUICK_TRIALS_MODE}" != "fixed" && "${QUICK_TRIALS_MODE}" != "budget" ]]; then
	echo "[tune_dnabert_pair.sh] QUICK_TRIALS_MODE must be fixed|budget." >&2
	exit 1
fi
if [[ "${SEARCH_ALGO}" != "random" && "${SEARCH_ALGO}" != "history_guided" ]]; then
	echo "[tune_dnabert_pair.sh] SEARCH_ALGO must be random|history_guided." >&2
	exit 1
fi
if [[ "${TRIAL_PROCESS_MODE}" != "subprocess" \
	&& "${TRIAL_PROCESS_MODE}" != "persistent_quick" \
	&& "${TRIAL_PROCESS_MODE}" != "persistent_all" ]]; then
	echo "[tune_dnabert_pair.sh] TRIAL_PROCESS_MODE must be "\
		"subprocess|persistent_quick|persistent_all." >&2
	exit 1
fi
if [[ "${QUICK_COMPILE_MODE}" != "off" && "${QUICK_COMPILE_MODE}" != "on" \
	&& "${QUICK_COMPILE_MODE}" != "auto" ]]; then
	echo "[tune_dnabert_pair.sh] QUICK_COMPILE_MODE must be off|on|auto." >&2
	exit 1
fi
if [[ "${FULL_COMPILE_MODE}" != "off" && "${FULL_COMPILE_MODE}" != "on" \
	&& "${FULL_COMPILE_MODE}" != "auto" ]]; then
	echo "[tune_dnabert_pair.sh] FULL_COMPILE_MODE must be off|on|auto." >&2
	exit 1
fi
if ! [[ "${QUICK_TRIALS}" =~ ^[0-9]+$ ]] || [[ "${QUICK_TRIALS}" -le 0 ]]; then
	echo "[tune_dnabert_pair.sh] QUICK_TRIALS must be a positive integer." >&2
	exit 1
fi
if ! [[ "${HISTORY_TOP_N}" =~ ^[0-9]+$ ]] || [[ "${HISTORY_TOP_N}" -le 0 ]]; then
	echo "[tune_dnabert_pair.sh] HISTORY_TOP_N must be a positive integer." >&2
	exit 1
fi
if ! [[ "${MIN_QUICK_TRIALS}" =~ ^[0-9]+$ ]] || [[ "${MIN_QUICK_TRIALS}" -le 0 ]]; then
	echo "[tune_dnabert_pair.sh] MIN_QUICK_TRIALS must be a positive integer." >&2
	exit 1
fi
if ! [[ "${TARGET_TIME_BUDGET_MINUTES}" =~ ^[0-9]+$ ]]; then
	echo "[tune_dnabert_pair.sh] TARGET_TIME_BUDGET_MINUTES must be an integer." >&2
	exit 1
fi
if ! [[ "${TOTAL_TIME_BUDGET_MINUTES}" =~ ^[0-9]+$ ]]; then
	echo "[tune_dnabert_pair.sh] TOTAL_TIME_BUDGET_MINUTES must be an integer." >&2
	exit 1
fi
if ! [[ "${QUICK_TRIAL_SEC_FALLBACK}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_dnabert_pair.sh] QUICK_TRIAL_SEC_FALLBACK must be a positive number." >&2
	exit 1
fi
if ! [[ "${GUIDED_RANDOM_FRACTION}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_dnabert_pair.sh] GUIDED_RANDOM_FRACTION must be numeric in [0,1]." >&2
	exit 1
fi
if ! awk -v x="${GUIDED_RANDOM_FRACTION}" 'BEGIN{exit !(x>=0 && x<=1)}'; then
	echo "[tune_dnabert_pair.sh] GUIDED_RANDOM_FRACTION must be in [0,1]." >&2
	exit 1
fi
if ! [[ "${GUIDED_MUTATION_RATE}" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
	echo "[tune_dnabert_pair.sh] GUIDED_MUTATION_RATE must be numeric in [0,1]." >&2
	exit 1
fi
if ! awk -v x="${GUIDED_MUTATION_RATE}" 'BEGIN{exit !(x>=0 && x<=1)}'; then
	echo "[tune_dnabert_pair.sh] GUIDED_MUTATION_RATE must be in [0,1]." >&2
	exit 1
fi
...
if [[ "${QUICK_TRIALS_MODE}" == "budget" ]]; then
	if [[ "${TARGET_TIME_BUDGET_MINUTES}" -le 0 && "${TOTAL_TIME_BUDGET_MINUTES}" -le 0 ]]; then
		echo "[tune_dnabert_pair.sh] budget mode requires TARGET_TIME_BUDGET_MINUTES>0 "\
...
echo "[tune_dnabert_pair.sh] species=${SPECIES}"
echo "[tune_dnabert_pair.sh] targets=${TARGET_LIST[*]}"

for TARGET in "${TARGET_LIST[@]}"; do
	TARGET_MODEL_NAME="${MODEL_NAME}_pair"
	TARGET_TUNING_MODEL_NAME="${TARGET_MODEL_NAME}"
	if [[ "${TRUNC_MODE}" == "on" ]]; then
	...
	if [[ "${QUICK_TRIALS_MODE}" == "budget" ]]; then
		...
		if [[ "${TARGET_BUDGET_MINUTES}" -le 0 ]]; then
			echo "[tune_dnabert_pair.sh] target budget computed as 0 minutes. "\
		...
		if ! estimate_result="$(
			...
			echo "[tune_dnabert_pair.sh] failed to estimate quick trials from budget." >&2
			echo "[tune_dnabert_pair.sh] estimator detail: ${estimate_result}" >&2
			exit 1
		fi
		IFS=$'\t' read -r \
			QUICK_TRIALS_TARGET \
		...
		echo "[tune_dnabert_pair.sh] target=${TARGET} budget=${TARGET_BUDGET_MINUTES}min "\
	...
	if search_space_resolved="$(
		...
		if ! target_space_json="$(
			...
			echo "[tune_dnabert_pair.sh] failed to parse search-space file: ${search_space_path}" >&2
			echo "[tune_dnabert_pair.sh] parse detail: ${target_space_json}" >&2
			exit 1
		fi
		TARGET_SEARCH_SPACE_JSON="${target_space_json}"
		echo "[tune_dnabert_pair.sh] target=${TARGET} search_space_file=${search_space_path}"
	else
		search_space_status=$?
		if [[ "${search_space_status}" -eq 2 ]]; then
			exit 1
		fi
		echo "[tune_dnabert_pair.sh] target=${TARGET} search_space_file=<embedded_${TARGET}>"
	fi
...
JSON
	...
	echo "[tune_dnabert_pair.sh] target=${TARGET}"
	echo "[tune_dnabert_pair.sh] output_dir=${OUTPUT_DIR}"
	if ! intronmodel_run_with_process_title \
		...
		echo "[tune_dnabert_pair.sh] target=${TARGET} failed start=${TARGET_START_EPOCH} "\
	...
	target_elapsed_seconds=$((SECONDS - TARGET_START_SECONDS))
	target_elapsed_hms="$(format_elapsed "${target_elapsed_seconds}")"
	echo "[tune_dnabert_pair.sh] target=${TARGET} done start=${TARGET_START_EPOCH} "\
